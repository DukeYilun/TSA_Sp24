...
summary(Model3)
plot(Model3)
#Model 4: Naive on deseas data (consider without seasonality)
Model4 <- naive(ts_electricity_price[,"All.sectors"], h=12)
summary(Model4)
plot(Model4)
...
```{r}
# "hold out" argument is used for push your forecast back some period so that you can compare the forecast with acknowledged data  
# Model 9:  SARIMA on original data
Model9 <- auto.arima(ts_electricity_price[,"All.sectors"])

Model9_forca <- forecast(Model9, h=12)
plot(Model9_forca)
autoplot(ts_electricity_price[,"All.sectors"], series = "Original")+
  autolayer(Model9$fitted, series = "SARIMA-Model9")+
  autolayer(Model9_forca$mean, series = "SARIMA-M9forca")

# Model 10:  ARIMA on deseasonal data
Model10 <- auto.arima(deseasonal_allsectors_price, seasonal = FALSE)
Model10_forca <- forecast(Model10, h=12)
plot(Model10_forca)
autoplot(deseasonal_allsectors_price, series = "Deseasonal")+
  autolayer(Model10$fitted, series = "SARIMA-Model10")+
  autolayer(Model10_forca$mean, series = "SARIMA-M10forca")
```

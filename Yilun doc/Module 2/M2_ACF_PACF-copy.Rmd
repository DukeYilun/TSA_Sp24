...
# after the . and /, press Tab will show you a small box of directory and keep press Tab to choose doc
...
my_date <- my(my_date)  #function my from package lubridate, my is month year
head(my_date)
...
```{r}
#Professor's way to transfer TS data, can also use [,"HP1] to replace [,2], start = c(Year, Month, Day), start will automatically take the first column, so we need to order the data if Time is not the first column.
...
#The blue dotted line is significance line, means ACF/PACF higher than blue line can be consdiered in building TSA model.
HP1_pacf <- pacf(ts_inflow_data, lag.max = 60)
...
#"lag" means the number of observation show in the acf chart, code 'lag.max' to set lag
HP1_acf = Acf(ts_inflow_data, lag.max = 60)
#High lag with a strong ACF/PACF(higher than 0.5) means those those periods are all needed to be considered and cannot be ingored.
#Sometimes intermediate variable may influence acf, Vt may have relationship with Vt-5 nor Vt-1. Then to get significant lag, we need to use PACF Partial Autocorrelation Function.
HP1_pacf=
